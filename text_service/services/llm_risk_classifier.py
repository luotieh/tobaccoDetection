...
class LlmRiskClassifier:
    ...
    def build_prompt(self, text: str, hits: list[KeywordHit], contacts: list[TextEntity], context: str = "") -> str:
        ...
        return (
            ...
            "评论场景特别注意：在售卖类帖子上下文中，简短的购买/交易口语即属高意图，不要因其简短而判为 normal_discussion——\n"
            "  · 询问下单/购买/交易：怎么下单、哪里下单、怎么买、怎么卖、哪里买、要一条、私聊我 → sale_intent / trade_lead（≥0.75）\n"
            "  · 询问价格/数量：多少钱、几盒多少、一条一箱多少、有没有X、有X吗、来X盒 → price_quantity（≥0.7），并视情给 trade_lead\n"
            "  · 批发/代发/一手/厂家/面交 等渠道词 → sale_intent / trade_lead（≥0.8）\n"
            f"允许标签：{labels}\n"
