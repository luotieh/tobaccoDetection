...
class TextRiskPipeline:
    ...
    def infer_content(self, req: ContentInferRequest) -> ContentInferResult:
        ...
        risk_types = sorted({typ for item in results for typ in item.risk_types if typ != "normal_discussion"})
        field_results = [
        ...
        return ContentInferResult(
            ...
            risk_level=score_text([], [], [], [])[1] if not results else results[[item.text_score for item in results].index(max_score)].risk_level,
